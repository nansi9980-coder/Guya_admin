...
const STATUS_CONFIG: Record<string, { label: string; cls: string; icon: React.ElementType }> = {
  ...
  QUOTE_SENT: { label: 'Devis envoyé', cls: 'status-QUOTE_SENT', icon: FileText },
...
export default function DevisPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setDevis(Array.isArray(res) ? res : res.data || [])
    ...
    <div className="space-y-5">
      ...
      <Card>
        {loading ? (
          ...
          <EmptyState icon={<FileText className="w-6 h-6" />} title="Aucune demande" description="Aucun devis ne correspond à vos critères" />
        ) : (
          <Table>
            <Thead>
              <tr>
      ...
      {/* Detail Modal */}
      <Modal open={!!selected && !actionModal} onClose={() => setSelected(null)} title="Détail du devis" size="lg">
        {selected && (
          <div className="space-y-5">
            {detailLoading ? (
