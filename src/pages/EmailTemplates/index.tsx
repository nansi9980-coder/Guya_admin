...
import { Mail, Save, Edit, ChevronDown, ChevronRight, Info } from 'lucide-react'
          ...
          <div className="pt-4 space-y-3">
            <div>
              ...
              <div className="bg-muted/40 rounded-xl p-4 text-sm text-foreground leading-relaxed whitespace-pre-wrap max-h-48 overflow-y-auto">
                {template.bodyHtml?.replace(/<[^>]+>/g, '') || '(aucun contenu)'}
              </div>
            </div>
...
export function EmailTemplatesPage() {
  ...
  const handleEdit = (t: EmailTemplate) => {
    setEditing(t)
    const plainBody = t.bodyHtml?.replace(/<br\s*\/?>/gi, '\n').replace(/<[^>]+>/g, '') || ''
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const htmlBody = form.body
        .split('\n')
        .map(line => line.trim() ? `<p>${line}</p>` : '<br/>')
        .join('')
      await emailTemplatesApi.update(editing.slug, { subject: form.subject, bodyHtml: htmlBody })
      toast.success('Template mis à jour')
      setTemplates(ts => ts.map(t => t.slug === editing.slug ? { ...t, subject: form.subject, bodyHtml: htmlBody } : t))
      setEditing(null)
  ...
  return (
    <div className="space-y-5">
      ...
      <Card>
        <CardContent className="p-4 flex items-start gap-3">
          ...
          <p className="text-sm text-muted-foreground">
            Les variables entre <span className="font-mono text-primary">{'{{accolades}}'}</span> sont remplacées automatiquement par les vraies valeurs lors de l'envoi. Emails envoyés via <strong>Resend</strong> — assurez-vous que <code className="text-xs bg-muted px-1 py-0.5 rounded">RESEND_API_KEY</code> est configuré dans le backend.
          </p>
        </CardContent>
      </Card>
      ...
      {loading ? (
        ...
        <EmptyState icon={<Mail className="w-6 h-6" />} title="Aucun template" description="Les templates email apparaîtront ici après le seed de la base" />
      ) : (
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder="Écrivez votre message ici. Appuyez sur Entrée pour aller à la ligne."
                className="font-sans"
              />
              <p className="text-xs text-muted-foreground mt-1">Chaque ligne deviendra un paragraphe dans l'email.</p>
            </div>
