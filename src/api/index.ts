...
export const servicesApi = {
  ...
  toggle: (id: string) =>
    api.patch(`/services-content/${id}/toggle`).then(r => r.data),
  reorder: (items: { id: string; order: number }[]) =>
    api.patch('/services-content/reorder', { items }).then(r => r.data),
}

// ─── REALISATIONS ─────────────────────────────────────────────────────────
export const realisationsApi = {
  getAll: (params?: PaginationParams & { tag?: string; location?: string }) =>
  ...
  toggle: (id: string) =>
    api.patch(`/realisations/${id}/toggle`).then(r => r.data),
  toggleFeatured: (id: string) =>
    api.patch(`/realisations/${id}/featured`).then(r => r.data),
}
...
export const emailTemplatesApi = {
  ...
  update: (slug: string, data: Partial<EmailTemplate>) =>
